...
- **Assertions:** AssertJ library MUST be used for all assertions. Do not use standard JUnit assertions (like `assertEquals`, `assertNotNull`, etc.).
...
- **Coverage:** Every new feature must include corresponding tests before completion. If a behavior is fully exercised end-to-end by an IT (controller → service → mapper → response JSON), a separate unit test for each internal layer is **not required** unless the layer has complex logic that is not reachable via the happy path IT.
